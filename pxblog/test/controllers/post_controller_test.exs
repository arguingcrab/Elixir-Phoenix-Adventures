defmodule Pxblog.PostControllerTest do
  ...
  defp create_user do
    User.changeset(%User{}, %{email: "test@example.com", username: "testuser", password: "test1234", password_confirmation: "test1234"})
    |> Repo.insert
  end
  ...
  test "redirects when trying to edit a post for a different user", %{conn: conn, user: user} do
    other_user = User.changeset(%User{}, %{email: "test2@example.com", username: "testuser2", password: "test1234", password_confirmation: "test1234"})
    |> Repo.insert!
    post = build_post(user)
