defmodule Pxblog.PostControllerTest do
  ...
  test "redirects when trying to edit a post for a different user", %{conn: conn, role: role, post: post} do
    {:ok, other_user} = TestHelper.create_user(role, %{email: "test2@example.com", username: "testuser2", password: "test1234", password_confirmation: "test1234"})
  ...
  test "renders form for editing chosen resource when logged in as admin", %{conn: conn, user: user, post: post} do
    ...
    {:ok, admin} = TestHelper.create_user(role, %{username: "admin", email: "admin@example.com", password: "test1234", password_confirmation: "test1234"})
    conn =
